...
mkdir -p log/policy_vs_cache_size
for size in ${cache_size[@]}
do
    mkdir -p log/policy_vs_cache_size/$size
    for policy in ${cache[@]}
    do
        CACHE_POLICY=$policy
        CACHE_SIZE=$size        
        NS_LOG=nfd.ContentStore ./waf --run=3x3-ndn-lirs-grid &> log/policy_vs_cache_size/$size/$CACHE_POLICY-$CACHE_SIZE-$ZipfParam.log
